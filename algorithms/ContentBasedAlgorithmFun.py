...
# In[2]
articles_df = pd.read_csv('../resources/data/wordpress_tags.csv')
print('')
print('Out[2]:')
print(articles_df.head(5))

# In[3]. Csv con los usuarios, widget sy ratings
interactions_df = pd.read_csv('../resources/data/wordpress_ext.csv')
print('')
print('Out[3]:')
...
class ModelEvaluator:
    ...
    def get_not_interacted_items_sample(self, person_id, sample_size, seed=32):
    ...
    def evaluate_model_for_user(self, model, person_id):
        ...
        for item_id in person_interacted_items_testset:
            ...
            non_interacted_items_sample = self.get_not_interacted_items_sample(person_id, sample_size=EVAL_RANDOM_SAMPLE_NON_INTERACTED_ITEMS, seed=item_id_hs % (2 ** 32))
    ...
    def evaluate_model(self, model):
        ...
        print(detailed_results_df['interacted_count'].mean())
        
        global_precision_at_5 = detailed_results_df['hits@5_count'].sum() / (5.0*len(detailed_results_df))
        global_precision_at_10 = detailed_results_df['hits@10_count'].sum() / (10.0*len(detailed_results_df))
...
# In[18]
# myprofile = user_profiles['didr']
# print('')
# print('Out[18]')
# print(myprofile.shape)
# print(pd.DataFrame(sorted(zip(tfidf_feature_names,
#                              user_profiles['didr'].flatten().tolist()),
#                          key=lambda x: -x[1])[:20],
